    ...
    def generate_data(self):
        ...
        for class_idx in range(self.num_classes):
            for _ in range(self.num_samples_per_class):
                ...
                image_tensor = Resize((32,32), antialias=True)(torch.from_numpy(image).to(dtype=torch.float32).movedim(2,0))
    ...
    def generate_image(self, class_idx, image_size = None):
        ...
        if class_idx == 0:
            ...
            rf = rf_generator.gabor_kernel(
                ...
                theta=urand(0, torch.pi),
                sigma_x=urand(2, image_size[0] / 2),
                sigma_y=urand(2, image_size[1] / 2),
                center_offset=(
                    urand(-image_size[0] / 4, image_size[0] / 4),
                    urand(-image_size[1] / 4, image_size[1] / 4),
                ),
            ...
            rf = rf_generator.simple_edge(
                shape=image_size, theta=urand(0, torch.pi)
            )
            rf = rf_generator.greyscale_to_color(
                rf, urand(-1, 1), urand(-1, 1), urand(-1, 1)
            )
            ...
            rf = (
                ...
                + self.generate_image(2, image_size) * self.generate_image(3, image_size)
                + self.generate_image(4, image_size) * self.generate_image(5, image_size)
            )

        return rf + rf_generator.decorrelated_color_noise(
