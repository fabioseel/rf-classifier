...
class CNNClassifier(nn.Module):
    def __init__(
        ...
        self.convs = nn.Sequential(
            nn.Conv2d(3, conv_channels, kernel_size=kernel_size), nn.GELU()
        )
        for _ in range(conv_layers - 1):
            self.convs.append(
                nn.Conv2d(conv_channels, conv_channels, kernel_size=kernel_size)
            )
        ...
        self.fcs = nn.Sequential(
            nn.Linear(in_features=test_out.shape[1], out_features=fc_nodes), nn.GELU()
        )
